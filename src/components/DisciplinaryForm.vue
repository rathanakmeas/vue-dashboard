<template>
  <div class="form-grid">
    ...
    <div class="form-field">
      ...
      <SearchableSelect 
        v-model="formData.disciplinaryType" 
        :options="DISCIPLINARY_TYPES" 
        placeholder="ជ្រើសរើសឬស្វែងរក..."
        :class="{ 'error': errors.disciplinaryType }"
      />
      <span v-if="errors.disciplinaryType" class="error-message">{{ errors.disciplinaryType }}</span>
    </div>
    ...
    <div class="form-field full-width">
      ...
      />
      <small v-if="formData.disciplinaryDocument" style="color: #059669; margin-top: 0.5rem; display: block;">
        <i class="pi pi-check-circle"></i> {{ formData.disciplinaryDocument }}
      </small>
    </div>
  </div>
</template>
...
import { DOCUMENT_TYPES, DISCIPLINARY_TYPES } from '../constants/dropdown-options';
...
const schema = yup.object({
  ...
  disciplinaryType: yup.string().required('សូមជ្រើសរើសប្រភេទការដាក់ពិន័យ')
});
...
const handleFileUpload = (event) => {
  ...
  if (file) {
    formData.value.disciplinaryDocument = file.name;
  }
};
